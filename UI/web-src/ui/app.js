...
const uiApp = angular.module('maUiApp', ['ui.router', 'angular-loading-bar', 'ngMangoMaterial', 'ngMessages', 'mdColorPicker']);

uiApp
    .provider('maUiMenu', menuProvider)
...
uiApp.constant('MA_EVENT_LINK_INFO', {
    ...
    LICENSE_CHECK: {
        ...
        stateName: 'ui.system.modules',
        stateParams: (event) => ({})
    },
    EVENT_HANDLERS_LINK: {
        stateName: 'ui.datadistribution.eventHandlers',
        stateParams: (event) => {
...
uiApp.config([
    ...
    */
        ...
        const components = DOCS_PAGES.map((page) => page.id).filter((id) => id.indexOf('.') >= 0);
        ...
        components.forEach((item) => {
            ...
            if (matches) {
                ...
                let directiveName;
                if (matches.length > 3) directiveName = matches[3];
                ...
                const menuItem = {
                    name: 'ui.helps.docs.' + moduleName + '.' + name,
                    url: '/' + dashCaseUrl,
                    menuText: name,
                    templatePromise() {
                        return import(
                            /* webpackMode: "lazy-once", webpackChunkName: "ui.helps.docs" */
    ...
    function (
        ...
        $rootScope.setTitleText = function setTitleText() {
            if ($state.$current.menuText) {
                ...
                Translate.tr($state.$current.menuTr).then(
                    (text) => {
                        this.titleText = text;
                        if (uiSettings.titleSuffix) {
                            this.titleText += ' - ' + uiSettings.titleSuffix;
        ...
        $rootScope.$on('$stateChangeStart', (event, toState, toParams, fromState, fromParams) => {
            ...
            if (toState.name === 'ui.system.system') {
                event.preventDefault();
                $state.go('ui.system.system.systemInformation', toParams);
            }
            ...
            if (toState.name.indexOf('ui.helps.help.') === 0 || toState.name.indexOf('ui.helps.docs.') === 0) {
                const linkBetweenPages =
                    (toState.name.indexOf('ui.helps.help.') === 0 && $state.includes('ui.helps.help')) ||
                    (toState.name.indexOf('ui.helps.docs.') === 0 && $state.includes('ui.helps.docs'));
                ...
                if (openInSidebar) {
                    ...
                    // don't attempt closing help if this is the initial page load
        ...
        $rootScope.closeMenu = function () {
            //We close the sidenav submenu first and then the main main menu
            const sideNavSubmenu = $mdSidenav('submenu');
            if (sideNavSubmenu && sideNavSubmenu.isOpen()) {
                sideNavSubmenu.close();
            }
            $rootScope.navLockedOpen = false;
